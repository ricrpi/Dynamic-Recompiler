...
void Translate_CleanUp(code_seg_t* const codeSegment)
{
	...
	while (ins)
	{
		if (DR_NO_OP == ins->instruction)
		{
			ins = InstrFree(codeSegment, ins);
		}
		else if (MIPS_SLL == ins->instruction && ins->Rd1.regID == 0)	// MIPS NO_OP
		{
			ins = InstrFree(codeSegment, ins);
		}
		else if (MIPS_LUI == ins->instruction)
		{
			ins = InstrFree(codeSegment, ins);
		}
		else if (MIPS_CACHE == ins->instruction)
		{
			ins = InstrFree(codeSegment, ins);
		}
		else if (MIPS_SYSCALL == ins->instruction)
		{
			ins = InstrFree(codeSegment, ins);
		}
		else if (MIPS_BREAK == ins->instruction)
		{
			ins = InstrFree(codeSegment, ins);
		}
		else if (MIPS_SYNC == ins->instruction)
		{
			ins = InstrFree(codeSegment, ins);
		}
		else
