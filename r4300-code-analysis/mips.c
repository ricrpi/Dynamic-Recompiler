...
int32_t ops_validCodeSegment(uint32_t* uiMIPSword, uint32_t uiNumInstructions, uint32_t* pJumpToAddress)
{
	int x;

	for(x=0;x<uiNumInstructions;x++)
	{
		int op = ops_type(uiMIPSword[x]);
		...
		{
			if (pJumpToAddress) *pJumpToAddress = ops_JumpAddressOffset(uiMIPSword[x]);
			return x+1;
		}
...
int32_t ops_JumpAddressOffset(uint32_t uiMIPSword)
{
	...
	case 0x00: //printf("special\n");
		...
		switch(op2)
		{
		case 0x00: 	return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBLTZ   \t" B_OP "\n",x, OP_B(val)); return;	// I
		case 0x01: 	return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBGEZ   \t" B_OP "\n",x, OP_B(val)); return;	// I
		case 0x02: 	return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBLTZL\n",x); 	return;
		case 0x03: 	return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBGEZL\n",x); 	return;

		//case 0x09:  return (val<<16)/(1<<16); //printf(INDEX "\tBLTZAL \t" B_OP "\n",x, OP_B(val)); return;	// I and link
		case 0x10:  return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBLTZAL \t" B_OP "\n",x, OP_B(val)); return;	// I and link
		case 0x11: 	return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBGEZAL \t" B_OP "\n",x, OP_B(val)); return;	// I and link
		case 0x12: 	return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBLTZALL\t" B_OP "\n",x, OP_B(val)); return;	// I and link likely
		case 0x13: 	return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBGEZALL\t" B_OP "\n",x, OP_B(val)); return;	// I and link likely
		}break;

	case 0x02: 	return (uiMIPSword<<6)/(1<<6); //printf(INDEX "\tJ      \t" J_OP "\n", x, OP_J(val)); return;	// J
	case 0x03: 	return (uiMIPSword<<6)/(1<<6);   //printf(INDEX "\tJAL    \t" J_OP "\n", x, OP_J(val)); return;	// J
	case 0x04: 	return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBEQ    \t" I_OP "\n", x, OP_I(val)); return;	// I
	case 0x05: 	return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBNE    \t" I_OP "\n", x, OP_I(val)); return;	// I
	case 0x06: 	return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBLEZ   \t\n", x ); return;
	case 0x07: 	return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBGTZ   \t\n", x ); return;

	case 0x14: return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBEQL  \n",x); return;
	case 0x15: return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBNEL  \n",x); return;
	case 0x16: return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBLEZL \n",x); return;
	case 0x17: return (uiMIPSword<<16)/(1<<16); //printf(INDEX "\tBGTZL \n",x); return;
	}
	return 0x7FFFFFF;
}
...
void ops_decode(uint32_t x, uint32_t uiMIPSword)
{
	...
	case 0x00: //printf("special\n");
		...
		case 0x06: 	printf(INDEX "\tBLEZ   \t" B_OP "\n", x, OP_B(uiMIPSword)); return;	// I
		case 0x07: 	printf(INDEX "\tBGTZ   \t" B_OP "\n", x, OP_B(uiMIPSword)); return;	// I
